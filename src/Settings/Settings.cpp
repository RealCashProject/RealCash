...
const char DEFAULT_WALLET_FILE_NAME[] = "realcash.wallet";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("realcashwallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("realcashwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("realcashwallet.desktop");
  ...
  res = autorunSettings.contains("RealCashWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("RealCashWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.realcash.realcashwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=RealCash Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("RealCashWallet", appPath);
    } else {
      autorunSettings.remove("RealCashWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\realcash", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:realcash");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\realcash\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\realcash\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("realcashwallet.cfg"));
